...
(class Feature of Obj
	...
	(method (init param1)
		(cond 
			...
			(gLl5FtrInit (self perform: gLl5FtrInit))
		)
		...
		(cond 
			...
			(
				...
				(CueObj
					...
					theInvItem:
						(if
						(and gIconBar (== (pEvent message?) JOY_DOWNRIGHT) gInv)
				...
				(if
					...
					(gLarry
						setMotion: PolyPath approachX (+ (gLarry z?) approachY) CueObj
					)
				else
					(if (gUser canControl:) (gLarry setMotion: 0))
	...
	(method (doVerb theVerb)
		((if gLl5DoVerbCode else dftDoVerb)
	...
	(method (facingMe param1 &tmp temp0 temp1)
		(= temp0 (if argc param1 else gLarry))
		(if
			(>
				(= temp1
					(Abs
...
(instance dftDoVerb of Code
	...
	(method (doit param1 param2 param3 &tmp temp0 temp1)
		...
		(switch param1
			...
			(4
				(if (= temp1 (gInv at: param3))
					(proc255_4 950 1 (temp1 description?) temp0)
				)
			)
