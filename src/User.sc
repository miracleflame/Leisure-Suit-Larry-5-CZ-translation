	...
	(method (handleEvent pEvent &tmp pEventType temp1)
		...
		(if (pEvent type?)
			...
			(cond 
					...
					(cond 
						(
							(or
								...
								(and gLl5DirHandler (gLl5DirHandler handleEvent: pEvent))
				...
				((== pEventType evKEYBOARD) (if gLl5KDHandler (gLl5KDHandler handleEvent: pEvent)))
				(
				(and (== pEventType evMOUSEBUTTON) gLl5MDHandler) (gLl5MDHandler handleEvent: pEvent))
			)
		)
...
(class Ego of Actor
	...
	(method (put param1 param2 &tmp temp0 gIconBarState)
		(= gIconBarState (gIconBar state?))
		(gIconBar enable:)
		(if (self has: param1)
