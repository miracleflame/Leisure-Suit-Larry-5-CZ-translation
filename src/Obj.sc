...
(procedure (proc999_4 lEdge tEdge rEdge bEdge ptxOrObj pty)
	(return
		(and
			(<=
				lEdge
...
(class Script of Obj
	...
	(method (init theClient theCaller theRegister)
		(if (>= argc 1)
			((= client (= client theClient)) script: self)
			(if (>= argc 2)
				(= caller (= caller theCaller))
				(if (>= argc 3) (= register (= register theRegister)))
			)
		)
		...
		(cond 
			...
			(seconds
				...
				(!= lastSeconds (= theLastSeconds (GetTime 1)))
					(= lastSeconds (= lastSeconds theLastSeconds))
			...
			(
				(and
					...
					(<=
						(= ticks
							(= ticks (- ticks (Abs (- gCycleCnt lastTicks))))
						)
						0
					)
				)
		...
		(= script
			(= script
				(= timer
					(= timer
						(= client
							(= client (= next (= next (= caller (= caller 0)))))
						)
					)
				)
			)
